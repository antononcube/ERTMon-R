...
setMethod("addTimeGrid",
          ...
          function(eventRecords, object) {
            ...
            eventRecords <- 
              dplyr::left_join(
                dplyr::filter( dplyr::summarise( dplyr::group_by( dplyr::select( eventRecords, EntityID, ObservationTimeEpoch ), EntityID ), 
                                                 MostRecentTimeEpoch = max(ObservationTimeEpoch, na.rm = T) ), 
            ...
            eventRecords <- dplyr::mutate( eventRecords, 
                                          DiffToMaxObsTime = MostRecentTimeEpoch - ObservationTimeEpoch )
            ...
            eventRecords <- dplyr::mutate( eventRecords, TimeGridCell = floor( DiffToMaxObsTime / object@compSpec@variableToAggregationTimeInterval[Variable] ) )
...
setMethod("makeSparseMatrices",
          ...
          function(object, ...) {
            ...
            labelMat <- xtabs( ~ EntityID + Label, object@entityAttributes, sparse = T)
...
#' @description Applies a normalization function to long form contingency matrix data and stores the results is it is over group of records.
          ...
          function(object, aggrMRData) {
            ...
            object@groupAggregatedValues <-
              ddply( object@compSpec@parameters, "MatrixName", function(specRow) { 
                ...
                if ( specRow$Normalization[[1]] %in% c( "ByAgeAverage", "ByAgeMean") ) {
                  
                  ageAvg <- 
                    aggrMRData %>% 
                    dplyr::filter( MatrixName == specRow$MatrixName ) %>%
                    dplyr::inner_join( object@entityAttributes[, c("EntityID", "Age")], by = "EntityID" ) %>% 
                    dplyr::group_by( Age ) %>% 
                    dplyr::summarise( AgeAverage = mean(AValue) ) %>% 
                    dplyr::ungroup()
                  
                  if( nrow(ageAvg) == 0 ) { NULL }
                  else { rbind( object@groupAggregatedValues, cbind( MatrixName = specRow$MatrixName, ageAvg, stringsAsFactors = F ) ) }
...
setMethod("normalizeGroupsBySpec",
          ...
          function(specRow, object, matLongFormData, entityAttributes, normalizationFuncSpecToFunc ) {
            ...
            if ( specRow$Normalization[[1]] %in% c( "ByAgeAverage", "ByAgeMean") ) {
              ...
              if( is.null(ageAvg) || nrow(ageAvg) == 0 ) {
                
                warning( paste( "Missing accumulated averages data (after filtering.) for MatrixName=", specRow$MatrixName, "." ), call. = TRUE )
                ...
                matLongFormData %>% 
                  dplyr::filter( MatrixName == specRow$MatrixName ) %>% 
                  dplyr::inner_join( entityAttributes[, c("EntityID", "Age")], by = "EntityID" ) %>% 
                  dplyr::inner_join( ageAvg[, c("Age", "AgeAverage")], by = "Age" ) %>%
                  dplyr::group_by( EntityID ) %>% 
                  dplyr::mutate( AValue = AValue / AgeAverage ) %>%
